...
def return_concentration_day(animal, d):
    ...
    for session_name in session_names:
        ...
        # select triggered trials for this session
        session_ind = np.array([ind for ind in range(len(triggered_trial_info)) if triggered_trial_info[ind][1] == session_name])
        if len(session_ind) == 0:
            continue

        triggered_positions = [loaded_data["triggered_positions_baseoff"][ind] for ind in session_ind]
        triggered_info_session = np.array(triggered_trial_info)[session_ind]
        
        for triggered_position in triggered_positions:
            local_concentration_session, nonlocal_concentration_session = return_concentration_session(triggered_position, pos1d, pos2d, decode)
    ...
    return local_concentration_day, nonlocal_concentration_day
